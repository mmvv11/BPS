...
from .aggregators_tf2 import MyMeanAggregator
import numpy as np
...
class NGCF(object):
    def __init__(self, load_checkpoint, dataset_argv, architect_argv, adj_degree, num_samples,
                 ptmzr_argv, aggregator, act, neigh_drop_rate, l2_embed=1e-3,
                 ...
                 local_distill_mode='', contrastive_mode = '', layer_wise=None, layer_l2_mode=None, lambda_layer_l2=[0,0,0], lambda_contrastive=[0,0,0], user_l2_strength=None):
        '''
        ...
        with self.graph.as_default():
            ...
            # 개인별 l2 reg를 위한 값
            self.user_l2_strength = user_l2_strength
            ...
            self.old_u_i_adj_mat = tf.compat.v1.sparse_placeholder(tf.int32, shape=[self.old_num_user, self.old_num_item],
                                                         name='ui_1hop_mat_ph')
            self.old_i_u_adj_mat = tf.compat.v1.sparse_placeholder(tf.int32, shape=[self.old_num_item, self.old_num_user],
                                                         name='iu_1hop_mat_ph')
            ...
            self.old_item_gs = tf.compat.v1.placeholder(tf.float64,
                                              ...
                                              name='u_old_gs_ph')
            self.old_u_cluster_mat = tf.compat.v1.sparse_placeholder(tf.int32, shape=[self.k_centroids[0], self.old_num_user],
                                                           name='old_u_cluster_mat_ph')
            self.old_i_cluster_mat = tf.compat.v1.sparse_placeholder(tf.int32, shape=[self.k_centroids[1], self.old_num_item],
    ...
    def model_fn(self, scope):
        ...
        with tf.compat.v1.variable_scope(scope):
            if self.pretrain_data is None:
                self.user_embeddings = tf.Variable(initializer([self.num_user, self.input_dim], dtype=tf.float64), name='user_embedding')
                self.item_embeddings = tf.Variable(initializer([self.num_item, self.input_dim], dtype=tf.float64), name='item_embedding')
            ...
            self.user_medium_input_1 = tf.Variable(initializer([self.num_item, self.layer_dims[1]], dtype=tf.float64), name='user_medium_input_1')
            self.item_medium_input_1 = tf.Variable(initializer([self.num_user, self.layer_dims[1]], dtype=tf.float64), name='item_medium_input_1')
            self.user_medium_input_2 = tf.Variable(initializer([self.num_user, self.layer_dims[2]], dtype=tf.float64), name='user_medium_input_2')
            self.item_medium_input_2 = tf.Variable(initializer([self.num_item, self.layer_dims[2]], dtype=tf.float64), name='item_medium_input_2')
            ...
            if self.mse_reg_flag:
                ...
                user_reg = tf.reduce_sum(tf.math.squared_difference(user_embed, old_u_emb_matrix), 1) * self.u_mse_coef
                item_reg = tf.reshape(tf.reduce_sum(tf.math.squared_difference(item_embed, old_i_emb_matrix), 2),
                                      [-1]) * self.i_mse_coef
                ...
                u_diff_mat = tf.math.squared_difference(self.user_embeddings[:self.old_num_user],
                                                   self.old_user_embedding[:self.old_num_user])
                all_user_reg = tf.reduce_sum(u_diff_mat, 1) * self.u_mse_coef_dist_score
                i_diff_mat = tf.math.squared_difference(self.item_embeddings[:self.old_num_item],
            ...
            if self.inc_reg[1] > 0:
                if self.local_distill_mode == 'LSP_s':
                    ...
                    u_ia_kl = tf.reduce_mean(
                        tf.reduce_sum(tf.multiply(u_i_prob, tf.compat.v1.log(tf.divide(u_i_prob, self.old_user_bl_ls))), 1))
                    i_ua_kl = tf.reduce_mean(
                        tf.reduce_sum(tf.multiply(i_u_prob, tf.compat.v1.log(tf.divide(i_u_prob, self.old_item_bl_ls))), 1))
                    ...
                    loss_ui = tf.reduce_mean(tf.math.squared_difference(u_i_prod_new, self.ui_dist))
                    ...
                    loss_iu = tf.reduce_mean(tf.math.squared_difference(i_u_prod_new, self.iu_dist))
            ...
            if self.global_distillation_flag:
                ...
                u_ua_kl = tf.reduce_mean(tf.reduce_sum(tf.multiply(u_u_a_gs_matrix, tf.compat.v1.log(
                    tf.divide(u_u_a_gs_matrix, self.old_user_gs[:, :self.k_centroids[0]]))), 1))
                u_ia_kl = tf.reduce_mean(tf.reduce_sum(tf.multiply(u_i_a_gs_matrix, tf.compat.v1.log(
                ...
                i_ua_kl = tf.reduce_mean(tf.reduce_sum(tf.multiply(i_u_a_gs_matrix, tf.compat.v1.log(
                    tf.divide(i_u_a_gs_matrix, self.old_item_gs[:, :self.k_centroids[0]]))), 1))
                i_ia_kl = tf.reduce_mean(tf.reduce_sum(tf.multiply(i_i_a_gs_matrix, tf.compat.v1.log(
            ...
            # print("="*10)
            # print(f"user_rep: {user_rep}, user_rep shape: {user_rep.shape}")
            # print(tf.nn.l2_loss(user_rep))
            # print(f"item_rep: {item_rep}, item_rep shape: {item_rep.shape}")
            # print(tf.nn.l2_loss(item_rep))
            # print("=" * 10)

            # print("user_l2_strength", self.user_l2_strength)
            if self.user_l2_strength is not None:
                user_embeddings_squared = tf.square(self.user_embeddings)  # 유저 임베딩의 제곱 (u, 128)
                user_reg_terms = tf.reduce_sum(user_embeddings_squared, axis=1)  # 각 유저별 제곱합 (u,)
                user_reg_terms = user_reg_terms * self.user_l2_strength  # 유저별 정규화 강도 적용
                total_user_reg = tf.reduce_sum(user_reg_terms)  # 전체 유저에 대한 정규화 손실 합
                reg_loss = self.l2_embed * (total_user_reg + tf.nn.l2_loss(item_rep))
            else:
            ...
            if self.layer_wise:
                ...
                if self.layer_l2_mode:
                    for i in range(self.num_layers):
                        ...
                        tf.math.squared_difference(old_user_inputs[i][:self.old_num_user],
                                              user_inputs[i][:self.old_num_user]),-1))
                        l2_item_loss = -tf.reduce_sum(tf.reduce_mean(
                            tf.math.squared_difference(old_item_inputs[i][:self.old_num_item],
    ...
    def graphconv(self, scope, central_ids, user_embeddings, item_embeddings, tag, medium=False):
        with tf.compat.v1.variable_scope(scope):
            ...
            for i in range(self.num_layers - 2, -1, -1):
                ...
                if medium:
                    ...
                    if i == self.num_layers-2:
                        ...
                        # float64-float32
                        assigned_value = medium_input - tf.cast(medium_original, dtype=tf.float64)
            ...
            # embed = tf.cast(embed, dtype=tf.float64)
            if not medium:
                return output, embed
            else:
                # print(output.dtype, embed.dtype, medium_input_1.dtype, medium_input_2.dtype)
